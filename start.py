...
if __name__ == "__main__":
    ...
    if args.update:
        ...
        if not os.path.exists("/content/drive/MyDrive/aiweb/text-generation-webui"):
            install_dependencies()
            os.chdir(script_dir)

        # Check if a model has been downloaded yet
        if len([item for item in glob.glob('/content/drive/MyDrive/aiweb/text-generation-webui/models/*') if not item.endswith(('.txt', '.yaml'))]) == 0:
